...
int login(int client_sock, int usernum, int passwd) {
    int ret;
    pthread_mutex_lock(&(c2u_mutex[client_sock - 3]));
    pthread_mutex_lock(&(u2c_mutex[usernum]));
    // printf("LOGIN: usernum: %d, passwd: %d\n", usernum, passwd);
    if (client2user[client_sock - 3] != -1) {  // 이 클라이언트가 누구든지 이용중이면 로그아웃이 먼저
        ...
        client2user[client_sock - 3] = usernum;
        user2client[usernum] = client_sock - 3;
        ret = 1;
    } else if (password[usernum] != passwd) {
        // printf("Password INCORRECT\n");
        ret = -1;
    } else {
        // printf("USER %d at tid[%d] Login Success\n", usernum, client_sock - 3);
        client2user[client_sock - 3] = usernum;
        user2client[usernum] = client_sock - 3;
        ret = 1;
    }
    ...
    pthread_mutex_unlock(&(c2u_mutex[client_sock - 3]));
...
int reserve(int client_sock, int usernum, int seatnum) {
    ...
    int ret = -1;
    if (client2user[client_sock - 3] != usernum) {  // 이 클라이언트에 유저가 접속하지 않은 상태라면
        // printf("CLIENT %d HAS NO USER. LOGIN FIRST\n", client_sock - 3);
        ret = -1;
    } else if (user2client[usernum] != client_sock - 3) {  // 이 유저가 이 클라이언트에 로그인 안되어있다면
...
int logout(int client_sock, int usernum) {
    ...
    pthread_mutex_lock(&(u2c_mutex[usernum]));
    int ret;
    // printf("LOGOUT: CLIENT %d USERNUM %d\n", client_sock - 3, usernum);
    if (client2user[client_sock - 3] == -1) {  // 이 클라이언트가 이미 로그아웃이면
