...
void mac_address_toupper (char *mac) {
    int i=0;
    while (mac[i]){
...
int unpack_ethernet_header_frame (const u_char *packet) {
    ...
    printf("Source Mac: ");
    mac_address_toupper(ether_ntoa((struct ether_addr *) eth_frame->source_mac_addr));

    printf("Destination Mac: ");
    mac_address_toupper(ether_ntoa((struct ether_addr *) eth_frame->dest_mac_addr));
...
void dump (const unsigned char *data_buffer, const unsigned int length) { // WTF?
...
do_protocol (int ip_proto, const u_char *packet, int ipv, unsigned int header_len)
{
    ...
    if (ip_proto == 6) {
        printf("\tTCP Segment:\n");
        tcp_segment(packet);
        dump((packet + sizeof(struct tcp_header)), header_len); // print data
    }
    else if (ip_proto == 17) {
        printf("\tUDP Segment:\n");
        udp_segment(packet);
        dump((packet + sizeof(struct udp_header)), header_len); // print data
    }
    else if (ip_proto == 1) {
        printf("\tICMPv4 Packet:\n");
        icmp_packet(packet);
        dump((packet + sizeof(struct icmp_packet)), header_len); // print data
    }
    else if (ip_proto == 58){
...
{
    if (argc < 2) {
        fprintf(stderr, "Too Few Argument. PCAP filename is mandatory.\n");
        return -1;
    }
