...
void write_through_lpn(int bank, u32 halved_lpn, int* data){ // integer
	...
	int result = nand_write(bank, ppn/PAGES_PER_BLK, ppn%PAGES_PER_BLK, data, &halved_lpn); 
	// printf("stats.nand_write: %d->%d!!!\n",stats.nand_write,stats.nand_write+1);
	stats.nand_write++; // memcpy 32Byte && 4Byte
	next_ppn[bank]++;
}
