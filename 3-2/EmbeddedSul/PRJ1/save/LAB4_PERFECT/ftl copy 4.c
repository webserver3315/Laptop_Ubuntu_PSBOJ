...
/*
388번 줄대로 동작하게끔 수정해야함. 즉, isVPNfirstTime 은 필요없고, GTD[bank][vpn] == -1로 판별가능토록 짜야 문제가 안생길것임.
...
void ftl_open(){
	...
	FOR(bank, N_BANKS){
		...
		FOR(vpn, N_MAP_PAGES_PB) GTD[bank][vpn] = PAGES_PER_BLK*(N_USER_BLOCKS_PB + N_USER_OP_BLOCKS_PB) + vpn;
...
int lpn2ppn(int bank, int lpn){ // L2P[bank][lpn], 캐싱 안되어있으면 캐싱까지 해줌.
	// if(is_LPN_first_time[bank][lpn] == 1) return -1;
	int vpn = lpn2vpn(lpn);
...
{// bank, victim_slot 을 FLASH의 next_map_ppn에 FLUSH한다. map GC가 발생한다면 처리한다.
	...
	if(CMT_dirty[bank][victim_slot] == 1){// Dirty하면 nand에 써줘야함
		...
		assert(CMT_vpn[bank][victim_slot] >= 0 && GTD[bank][CMT_vpn[bank][victim_slot]] >= 0);
...
{// vpn에 해당하는 ppn을 GTD로 구하고, 그 ppn에 있는 데이터를 CMT의 free_slot에 무지성으로 적어줌.
	...
	if(tmp_spare != vpn) printf("tmp_spare == %d, vpn == %d\n", tmp_spare, vpn);
