...
/*
printf 주석들 깔끔하게 지웠고, isFirstTime 쓸데없이 또 체크하는 부분 지움.
input1~input8 전부 통과한다.
이제 CMT evict 용 LRU 알고리즘 구현 또는 캐싱 구현하면 된다.

stats 반영

어 씨발 데이터 캐시에 dirty정도는 구현할껄그랬나? ㅅㅂㅅㅂ 성능이 이게 더 좋으면 어떻게해
*/
...
void write_through_lpn(int bank, int lpn, int* data){
	...
	nand_write(bank, ppn/PAGES_PER_BLK, ppn%PAGES_PER_BLK, data, &lpn); stats.nand_write++;
	Next_Data_PPN[bank]++;
}
