...
void ftl_open()
{
	FOR(bank, N_BANKS){
		...
		FOR(vpn, N_MAP_PAGES_PB){
			GTD[bank][vpn]=PAGES_PER_BLK*(N_USER_BLOCKS_PB + N_USER_OP_BLOCKS_PB) + vpn;
		}
		GC_Trigger_Map_PPN[bank] = PAGES_PER_BLK*(N_USER_BLOCKS_PB+N_USER_OP_BLOCKS_PB+N_MAP_BLOCKS_PB+N_MAP_OP_BLOCKS_PB-1);
