...
/*
하 시발 혹시나 Clean Evict 가 문제였나 싶었는데 역시나 Dirty bit 설정문제였다.
CMT 의 dirty 가 Clean 되는 유일한 경우는 evict될 때, 즉 make_free_slot 뿐인데
use_free_slot 때도 clean 때리고 자빠져있었다.
CMT Cache 의 Dirty bit 가 Clean 될 때는 Eviction 될 때 뿐이야 CMT Cache 의 Dirty bit 가 Clean 될 때는 Eviction 될 때 뿐이야 CMT Cache 의 Dirty bit 가 Clean 될 때는 Eviction 될 때 뿐이야 CMT Cache 의 Dirty bit 가 Clean 될 때는 Eviction 될 때 뿐이야CMT Cache 의 Dirty bit 가 Clean 될 때는 Eviction 될 때 뿐이야 CMT Cache 의 Dirty bit 가 Clean 될 때는 Eviction 될 때 뿐이야 CMT Cache 의 Dirty bit 가 Clean 될 때는 Eviction 될 때 뿐이야 CMT Cache 의 Dirty bit 가 Clean 될 때는 Eviction 될 때 뿐이야
...
void show_whole_nand(){
	...
	for(int bb=0;bb<NBANKS;bb++) { 
		if(bb == 12 || bb == 13)
		printf("<Bank%d>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t",bb);
	}
	printf("\n");
	for(int bb=0;bb<NBANKS;bb++) if(bb == 12 || bb == 13) printf("Next_Data_PPN[%d] = %d\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t",bb,Next_Data_PPN[bb]);
	printf("\n");
	for(int bb=0;bb<NBANKS;bb++) if(bb == 12 || bb == 13) printf("GC_Trigger_Data_PPN[%d] = %d\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t",bb,GC_Trigger_Data_PPN[bb]);
	printf("\n");
	for(int bb=0;bb<NBANKS;bb++) if(bb == 12 || bb == 13) printf("get_free_blk_data[%d] = %d\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t",bb,get_free_blk_data(bb));
	printf("\n");
	for(int bb=0;bb<NBANKS;bb++) if(bb == 12 || bb == 13) printf("Next_Map_PPN[%d] = %d\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t",bb,Next_Map_PPN[bb]);
	printf("\n");
	for(int bb=0;bb<NBANKS;bb++) if(bb == 12 || bb == 13) printf("GC_Trigger_Map_PPN[%d] = %d\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t",bb,GC_Trigger_Map_PPN[bb]);
	printf("\n");
	for(int bb=0;bb<NBANKS;bb++) if(bb == 12 || bb == 13) printf("get_free_blk_map[%d] = %d\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t",bb,get_free_blk_map(bb));
	printf("\n");

	printf("\n");
	for(int vpn=0; vpn<N_MAP_BLOCKS_PB*PAGES_PER_BLK; vpn++){
		for(int bb=0;bb<N_BANKS;bb++){
			if((bb != 12 && bb != 13) || GTD[bb][vpn] == -1) continue;
		...
		FOR(page_offset, send_kazu){
			...
			if(page_offset == 0){ // 첫타
				...
				if(ppn == -1){
					...
					// printf("nand_read4, page_offset = %d\n",page_offset);
					nand_read(bank, ppn/PAGES_PER_BLK, ppn%PAGES_PER_BLK, data, &spare);
					FOR(i, sector_kazu) current_read_buffer[i] = data[i];
				}
			}
...
void write_through_lpn(int bank, int lpn, int* data){
	...
	// printf("NAND_WRITE(bank:%d, blk:%d, page:%d) := data = [ ", bank, ppn/PAGES_PER_BLK, ppn%PAGES_PER_BLK);
	// for(int i=0;i<7;i++){
	// 	printf("%2x, ", data[i]);
	// }
	// printf("%2x ]\n", data[7]);
...
int greedy_map_blk(int bank){ // 무지성으로 greedy victim을 선정함
	...
	// printf("GREEDY_MAP_BLK RETURN: %d\n",victim);
	return victim;
}
...
int lpn2ppn(int bank, int lpn){ // L2P[bank][lpn], 캐싱 안되어있으면 캐싱까지 해줌.
	...
	if(is_VPN_first_time[bank][vpn] == 1){
		...
		FOR(i,CMT_NSECT) {
			CMT_Data[bank][offset][i] = -1;
		}
	}
	// printf("return L2P[%d][%d] => %d <= CMT_Data[%d][%d][%d]\n", bank, lpn, CMT_Data[bank][offset][lpn%CMT_NSECT], bank, offset, lpn%CMT_NSECT);
...
void update_L2P(int bank, int lpn, int ppn){ // L2P[bank][lpn] = ppn;
	...
	// if(bank == 12 && ppn == 44) {
	// 	printf("HERE!!!!!!\n");
	// 	printf("CMT_Data[%d][%d] = [ ", bank, offset);
	// 	for(int i=0;i<7;i++){
	// 		printf("%d, ", CMT_Data[bank][offset][i]);
	// 	}
	// 	printf("%d ]\n", CMT_Data[bank][offset][7]);
	// }
...
{// bank, victim_slot 을 FLASH의 next_map_ppn에 FLUSH한다. map GC가 발생한다면 처리한다.
	...
	if(CMT_dirty[bank][victim_slot] == DIRTY){// Dirty하면 nand에 써줘야함
		...
		// if(isValid[bank][Next_Map_PPN[bank]] != VALID){
		// 	printf("isValid[%d][Next_Map_PPN[%d](==%d)] == %d\n", bank, bank, Next_Map_PPN[bank], isValid[bank][Next_Map_PPN[bank]]);
		// 	// show_whole_nand();
		// }
		// 꺼봄 한번
...
{// vpn에 해당하는 ppn을 GTD로 구하고, 그 ppn에 있는 데이터를 CMT의 free_slot에 무지성으로 적어줌.
	...
	if(ppn >= 0){
		...
		// printf("ppn:%d => bank: %d, blk: %d, page: %d\n", ppn, bank, ppn/PAGES_PER_BLK, ppn%PAGES_PER_BLK);
		// printf("limit: %d %d %d\n", N_BANKS, BLKS_PER_BANK, PAGES_PER_BLK);
		// printf("nand_read result = %d\n", result);
		FOR(i, CMT_NSECT){
			// printf("tmp_data[%d] == %d\n",i,tmp_data[i]);
			CMT_Data[bank][free_slot][i] = tmp_data[i]; 
			// if(bank == 12 && ppn == 44) {
				// printf("HERE2!!!!!!\n");
				// printf("CMT_Data[%d][%d] = [ ", bank, free_slot);
				// for(int i=0;i<7;i++){
					// printf("%d, ", CMT_Data[bank][free_slot][i]);
				// }
				// printf("%d ]\n", CMT_Data[bank][free_slot][7]);
			// }
			assert(tmp_data[i] >= -1);
			// 꺼봄
		}
