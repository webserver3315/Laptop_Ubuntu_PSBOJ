...
/*
방법은, nand_read 함수와 nand_write 함수를 대체하는 buffer_read, buffer_write 함수를 만든다. 그리고 끼워넣는다.
라고 생각했는데, write 되는 물리적 page의 위치를 buffer_read 가 알기 힘들어서 그냥 ftl_read, ftl_write 개조하는게 한계일듯.
원위치로.
*/
...
/**************************LAB 3*********************************/
int buffer_P2S [N_BANKS][N_PPNS_PB]; // PPN2Slot, 디폴트: -1
int buffer_Data[N_BANKS][N_BUFFERS][SECTORS_PER_PAGE]; // 1섹터==int, 각 버퍼는 페이지크기.
int buffer_S2P [N_BANKS][N_BUFFERS]; // Slot2PPN, 디폴트: -1
/****************************************************************/
...
/*********************************LAB 3**********************************/
void buffer_write(int bank, int blk, int page, void* data, void* spare);
void buffer_read(int bank, int blk, int page, void* data, void* spare);
...
// 버퍼에 써준다.
void buffer_write(int bank, int blk, int page, void* data, void* spare){ 
	int ppn = blk*PAGES_PER_BLK + page;
	int slot = buffer_P2S[bank][ppn];
	if(slot == -1){// Cache Miss
...
void buffer_read(int bank, int blk, int page, void* data, void* spare){
	...
	int slot = buffer_P2S[bank][ppn];
	if(slot == -1){// Cache Miss
		int empty_slot = get_empty_buffer_slot(bank);
		if(empty_slot == -1){ // BUFFER IS FULL, SO EVICT THE VICTIM SLOT
			int victim_slot = get_victim_buffer_slot(bank); assert(victim_slot != -1);
			
		}
	} // Cache Hit
}
int get_empty_buffer_slot(int bank){ // 실패시 -1 반환
	FOR(slot, N_BUFFERS){
		if(buffer_S2P[bank][slot] == -1) return slot;
	}
	return -1;
}
void slot_flush(int bank, int victim_slot){
	int ppn = buffer_S2P[bank][victim_slot]; assert(ppn != -1)

}
int get_victim_buffer_slot(int bank){ // 원래는 LRU로 구현.
	return 0;
}
...
static void garbage_collection(u32 bank)
{
	...
	FOR(page_offset, PAGES_PER_BLK){
		...
		if(immigrants_cnt == 0){ // 0번 page 를 write 할때만 발동
			age_update(bank, free_pblk);
		}
...
{	
	...
	if(send_kazu == 1){
		...
		if(buffer_L2P[bank][lpn] != -1){ // IF lpn Also Buffered, 최신형을 덮어쓴다.
			FOR(i, nsect){
				if(buffer_isValid[buffer_L2P[bank][lpn]][i+first_lba_offset] != -1){
					current_read_buffer[i] = buffer_pblk[buffer_L2P[bank][lpn]][i+first_lba_offset];
				}
			}
		}
	}
		...
		FOR(page_offset, send_kazu){
			...
			if(page_offset == 0){ // 첫타
				...
				if(buffer_L2P[bank][lpn] != -1){
					/**************************여기까지 검토함************************/
					FOR(i, sector_kazu){
						// if(buffer_isValid[buffer_L2P[bank][lpn]])
					}
				}
