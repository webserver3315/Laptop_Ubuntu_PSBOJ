...
/*
205번줄까지 검토하다가 lab3 짜놓은 꼬라지보고 현타옴.

하 씨발 lab3 그냥 아예 따라가는게 낫나? lab3 디버깅하기 VS 아예 새로짜기
도찐개찐삘인데 ㅅㅂ
디버깅쪽으로 가자.
*/
...
void buffer_write(int bank, int blk, int page, int slot){
	nand_write(bank, blk, page, buffer_Data[bank][slot], &buffer_Spare[bank]);
}
void buffer_read(int bank, int blk, int page, int slot){
	nand_read(bank, blk, page, buffer_Data[bank][slot], &buffer_Spare[bank]);
}
