...
그런 자잘한건 일단 무시되어있다.
buffer_L2S 제거 완료, 메모리 확보

씨발 실패했다실패했다실패했다실패했다실패했다실패했다실패했다실패했다실패했다실패했다실패했다실패했다실패했다실패했다
머리가 너무 피곤해서 그런가 dirty 구현조차 제대로 안된다.
일단 보존하고 보존본으로 테스트돌리고 리포트 다듬고 제출박자.
*/
...
int sim_buffer_L2S(int bank, int lpn){
	FOR(slot, N_BUFFERS){
		if(buffer_S2L[bank][slot] == lpn) return slot;
	}
	return -1;
}
...
int get_free_data_slot(int bank){
	FOR(i, N_BUFFERS){
...
void show_score(){
	printf("<SCORE: DFTL + Caching RESULT>\n");
	printf("gc_cnt: %d\n", stats.gc_cnt);
	printf("map_gc_cnt: %d\n", stats.map_gc_cnt);
	printf("host_write: %d\n", stats.host_write);
	printf("nand_write: %d\n", stats.nand_write);
...
static void garbage_collection(u32 bank)
{
	...
	FOR(page_offset, PAGES_PER_BLK){
		...
		if(isValid[bank][victim_ppn] == INVALID) continue;
...
{	
	...
	if(send_kazu == 1){
		...
		dst_slot = sim_buffer_L2S(bank,lpn);
		if(dst_slot != -1){ // Cache Hit
			stats.cache_hit++;
			FOR(i, sector_kazu) current_read_buffer[i] = buffer_Data[bank][sim_buffer_L2S(bank,lpn)][first_lba_offset + i];
		}else{// Cache Miss
		...
		FOR(page_offset, send_kazu){
			...
			if(page_offset == 0){ // 첫타
				...
				if(dst_slot != -1){
					stats.cache_hit++;
					FOR(i, sector_kazu) current_read_buffer[i] = buffer_Data[bank][sim_buffer_L2S(bank,lpn)][i];
				}else{
					stats.cache_miss++;
					if(ppn == -1){
...
void make_free_data_slot(int bank, int victim_slot){
	if(buffer_Dirty[bank][victim_slot] == DIRTY){
		...
		if(before_ppn != -1) isValid[bank][before_ppn] = INVALID;
		update_L2P(bank, buffer_S2L[bank][victim_slot], Next_Data_PPN[bank]);
		nand_write(bank, Next_Data_PPN[bank]/PAGES_PER_BLK, Next_Data_PPN[bank]%PAGES_PER_BLK, buffer_Data[bank][victim_slot], &buffer_S2L[bank][victim_slot]); stats.nand_write++;

		Next_Data_PPN[bank]++;
	}
	buffer_S2L[bank][victim_slot] = -1;
	buffer_Dirty[bank][victim_slot] = CLEAN;
}

// data: 쓰고자하는 페이지 전체. 변동사항이 어디인지는 모름.
void write_through_lpn(int bank, int lpn, int* data){ // 딱 페이지단위로만 쓰기
	int dst_slot = sim_buffer_L2S(bank,lpn);
	if(dst_slot == -1){ // Cache Miss
		stats.cache_miss++;
		if(get_free_data_slot(bank) == -1){ make_free_data_slot(bank, get_victim_data_slot(bank)); }
		int free_slot = get_free_data_slot(bank); assert(free_slot != -1);

		FOR(i, SECTORS_PER_PAGE) buffer_Data[bank][free_slot][i] = data[i];
		buffer_Spare[bank][free_slot] = lpn;
		buffer_S2L[bank][free_slot] = lpn;
		buffer_Dirty[bank][free_slot] = DIRTY;
		
		dst_slot = get_free_data_slot(bank);
	}else{
		// Cache Hit
		assert(dst_slot != -1);
		...
		buffer_Dirty[bank][dst_slot] = DIRTY;	
...
{ // stats.cache_hit/miss 는 write_through_lpn 에서 업데이트.
	...
	if(send_kazu == 1){
		...
		if(sim_buffer_L2S(bank,lpn) != -1){// Cache Hit
			FOR(i, SECTORS_PER_PAGE) data[i] = buffer_Data[bank][sim_buffer_L2S(bank,lpn)][i];
		}
		else if(ppn == -1){ // Empty
...
void ftl_open(){
	...
	FOR(bank, N_BANKS){
		FOR(buf, N_BUFFERS) {
			buffer_Dirty[bank][buf] = CLEAN;
			// buffer_Dirty[bank][buf] = DIRTY;
...
{// bank, victim_slot 을 FLASH의 next_map_ppn에 FLUSH한다. map GC가 발생한다면 처리한다.
	...
	CMT_dirty[bank][victim_slot] = CLEAN;
}
...
{// vpn에 해당하는 ppn을 GTD로 구하고, 그 ppn에 있는 데이터를 CMT의 free_slot에 무지성으로 적어줌.
	...
	CMT_dirty[bank][free_slot] = DIRTY;
}
