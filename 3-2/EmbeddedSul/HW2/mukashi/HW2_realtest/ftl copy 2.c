...
int page_map_table [N_BANKS][N_USER_BLOCKS_PB * PAGES_PER_BLK]; // lpn 이 쓰인다.
...
***************************************/
	...
	int spare_first_ppn = gc_ppn[bank];
	int spare_blk = spare_first_ppn/PAGES_PER_BLK;
	/*
	1. victim 블록 선정
	...
	*/
	int tmp_data[PAGES_PER_BLK][8];
	int tmp_spare[PAGES_PER_BLK];

	for(int i=0;i<PAGES_PER_BLK;i++){
		if(isValid[bank][victim_first_ppn+i] == 1){
			nand_read(bank,victim_blk, i, tmp_data[valid_cnt], &tmp_spare[valid_cnt]); // 새로 이사간 ppn으로 옮길 data, spare 임시저장
			page_map_table[bank][tmp_spare[valid_cnt]]=victim_blk*PAGES_PER_BLK+i; // table 변경
			valid_cnt++;
		}
	}
	...
	for(int i=0;i<PAGES_PER_BLK;i++){
		if(isValid[bank][victim_first_ppn + i] == 0) {
			isValid[bank][victim_first_ppn + i] == 1; // Make VALID
			continue;
		}
		int data[8];
		int spare;
		...
		stats.gc_write++;
		nand_write(bank, spare_blk, valid_cnt, data, &spare);
		valid_cnt++;
	}
	gc_ppn[bank]=victim_first_ppn; // Victim 은 새로운 Spare Block 이 된다.
	return;
}
