...
***************************************/
    ...
    for(int i=0;i<PAGES_PER_BLK;i++){
        ...
        if(isValid[bank][victim_ppn] == VALID){ // 이민 시작
			...
			isValid[bank][free_first_ppn+immigrants_cnt] = VALID;
            immigrants_cnt++;
        }
    }
    for(int i=0;i<PAGES_PER_BLK;i++){
        isValid[bank][victim_first_ppn + i] = EMPTY;
    }
...
void ftl_read(u32 lba, u32 nsect, u32 *read_buffer) { //read 에서 lba 를 lpn 을 안거치고 ppn으로 변환했다.
    ...
    if(send_kazu==0){
        ...
        if(L2P[bank][halved_lpn]==-1 || isValid[bank][L2P[bank][halved_lpn]] == EMPTY){// Empty
...
void write_through_lpn(int bank, u32 halved_lpn, int* data){ // integer
	...
	if(L2P[bank][halved_lpn]!=-1){// 덮어쓰기라면
        ...
        isValid[bank][L2P[bank][halved_lpn]]=TRASH;
    }// VALID 말고 TRASH, EMPTY 일때도 케이스 만들어야하려나? 근데 L2P가 애초에 TRASH를 가리키고 있으면 안되는데
	
    // printf("next_ppn[%d] == %d\n",bank,next_ppn[bank]);
    L2P[bank][halved_lpn]=next_ppn[bank];
	if(isValid[bank][next_ppn[bank]] != EMPTY) printf("ERROR3 result = - to be Written, PPN Must be Empty\n");
	isValid[bank][next_ppn[bank]] = VALID;
    // printf("nand_write bank:%d\n blk:%d\n page=%d\n",bank,halved_lpn/PAGES_PER_BLK,halved_lpn%PAGES_PER_BLK);
    // printf("data = \'%d\'\n",data[0]);
    int result = nand_write(bank, next_ppn[bank]/PAGES_PER_BLK, next_ppn[bank]%PAGES_PER_BLK, data, &halved_lpn); stats.nand_write++; // memcpy 32Byte && 4Byte
