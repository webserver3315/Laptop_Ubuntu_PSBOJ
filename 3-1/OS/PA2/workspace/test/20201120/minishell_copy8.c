...
/*
example 을 참고하여, multiple pipeline 과 redirection 을 질서있게 구현하였다.
이제 error 명세에 맞도록 수정하고,
뚫린 부분 없는지 확인하고,
SIGINT, SIGSTP 면역 넣고,
background process 구현 및 동작 넣으면 된다.

추가적으로 어그로끌릴까봐 switch 문을 if 문으로 바꿨다.

ISSUE: ls -al /etc | sort -r > ls_sorted.txt 작동안함
ISSUE: ls -al /etc | sort -r 작동안함
*/
...
void eval(char *raw_cmd) {
    ...
    int input_redirection, output_redirection, status;
    int l_pipe[2], r_pipe[2];
    input_redirection = output_redirection = 0;
