...
/*
inituvm, allocuvm, copyuvm 3개 함수에서만 swap-out 가능한, 즉 lru 에 탑재해야할 페이지가 생성된다.
이 3개 수문장만 지키면 된다.
*/
...
deallocuvm(pde_t *pgdir, uint oldsz, uint newsz) // exit할때 이게 호출되는데, 해당 exit하는 프로세스가 kalloc한 모든 메모리를 kfree해주는 코드, 즉 bitmap, lru 전부 해제해줘야함
{
  ...
  for(; a  < oldsz; a += PGSIZE){
    ...
    if(!pte)
      ...
      //   struct page *cur = get_linked_list(i);
      //   if (is_struct_page_available(cur) == 0)
      //     continue;
      //   if(cur->vaddr ==(char*) a){
      //     lru_delete(cur);
      //   }
